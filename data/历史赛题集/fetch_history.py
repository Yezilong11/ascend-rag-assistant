# 文件名: fetch_history.py
# 放置位置: 历史赛题集/ 文件夹内
# 功能: 爬取64个竞赛的历史赛题，生成64个Markdown文件到当前文件夹
...
# 问题网址记录文件
PROBLEM_URLS_FILE = "problem_urls_history.txt"
...
def fetch_history_data(contest):
    ...
    try:
        ...
        if history_content:
            history_md += "## 相关赛题内容\n\n"
            for para in history_content[:30]:  # 限制数量
                history_md += f"{para}\n\n"
        else:
...
def main():
    ...
    print("=" * 60)
    print("【历史赛题集】爬虫开始运行")
    print(f"目标: 为 {len(CONTEST_LIST)} 个竞赛生成历史赛题Markdown文件")
