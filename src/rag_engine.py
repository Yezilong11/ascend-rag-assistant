...
try:
    ...
    print("未安装sentence-transformers，重排序功能不可用。请运行: pip install sentence-transformers")
...
class Reranker:
    ...
    def __init__(self, model_name: str = "bge-reranker-v2-m3", model_dir: str = "./models", device: str = None):
        ...
        if model_name not in PREDEFINED_RERANKERS:
            print(f"未知的重排序模型: {model_name}，使用默认 bge-reranker-v2-m3")
        ...
        if os.path.exists(local_model_path):
            ...
            if MODELSCOPE_AVAILABLE:
                ...
                try:
                    ...
                    print(f"ModelScope下载失败: {str(e)}，尝试从HuggingFace加载")
                    model_path = model_id
            else:
                model_path = model_id
...
class RAGAssistant:
    ...
    def __init__(self, knowledge_base, model_key="qwen2-1.5b", model_dir="./models", 
        ...
        if os.path.exists(local_model_path):
            ...
            if MODELSCOPE_AVAILABLE:
                ...
                try:
                    ...
                    print(f"ModelScope下载失败: {str(e)}，回退到HuggingFace")
                    model_path = model_id
            else:
                model_path = model_id
                print(f"本地模型未找到，正在从HuggingFace下载: {model_id}...")
        ...
        try:
            ...
            # 如果模型在meta设备上，使用to_empty()方法
            print(f"模型在meta设备上，使用to_empty()方法移动到{device}")
        ...
        if self.use_reranker and RERANKER_AVAILABLE:
            try:
                ...
                print(f"重排序器加载失败: {str(e)}，将不使用重排序")
